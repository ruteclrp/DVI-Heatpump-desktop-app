...
- `npm run dist:win` builds the app and packages a Windows installer into `release/`.
- The expected installable artifact is an `.exe` file similar to `release/DVI Heatpump Setup <version>.exe`.
...
- `DVI_BRIDGE_DISCOVERY_PATH`: override the probe path used to test whether a bridge is reachable
